...
function weather(data){
  ...
  weatherIcon.alt = `Weather: ${data.current.weather[0].description}`
  ...
  temp.textContent = `Temp: ${data.current.temp.toFixed()}°F`
  current.appendChild(temp)
  
  const feelsLike = document.createElement('li')
  feelsLike.textContent = `Feels like: ${data.current.feels_like.toFixed()}°F`
  ...
  dewPoint.textContent = `Dew Point: ${data.current.dew_point.toFixed()}°F`
