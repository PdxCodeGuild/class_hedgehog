...
# Remember there isn't any "perfect" way to identify a word in english (acronym's, mr/ms, contractions, etc), try to find rules that work best.

# Find a book on [Project Gutenberg](http://www.gutenberg.org) and navigate to the plain-text version.
...
# 2. Your dictionary will have words as `keys` and counts as `values`. 
...
def main():
    ...
    text = response.text.lower().replace("\n", "").replace("\r", "") 
    for x in string.punctuation:
        text_list = text.replace(x, "").split(" ")
